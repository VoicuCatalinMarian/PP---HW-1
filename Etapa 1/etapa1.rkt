...
(define (get-pref-list-helper pref person acc)
  (cond
    ((null? pref) acc)
    ((eq? person (car (car pref))) (get-pref-list-helper (cdr pref) person (cons (car pref) acc)))
    (else (get-pref-list-helper (cdr pref) person acc))))

(define (get-pref-list pref person)
  (cdar (get-pref-list-helper pref person '())))
...
(define (preferable? pref-list x y)
  (or (null? pref-list)
      (eq? (car pref-list) x)
      (and (not (eq? (car pref-list) x))
           (not (eq? (car pref-list) y))
...
(define (get-partner engagements person)
  (cond
    ((null? engagements) #f)
    ((eq? person (car (car engagements))) (cdr (car engagements)))
