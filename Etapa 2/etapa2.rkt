...
(define (get-pref-list pref person)
  (foldl (lambda (x acc) (if (member person x) (cdr x) acc)) null pref))
...
(define (preferable? pref-list x y)
  ((lambda (a b) (if (< (length a) (length b) ) #f #t)) (member x pref-list) (member y pref-list)))
...
(define (find-first p L)
  (foldl (lambda (x acc) (if (and (not acc) (p x)) x acc)) #f L))
...
(define (get-partner engagements person)
  (define (get-partner-helper engagements)
    (and (equal? person (car engagements)) (cdr engagements)))
...
(define (change-first p L val)
  (define (change-first-helper L acc)
    (cond
      ((null? L) '())
      ((and (not acc) (p (car L))) (cons val (cdr L)))
      (else (cons (car L) (change-first-helper (cdr L) acc)))))
  (change-first-helper L #f))
...
(define (update-engagements engagements p1 p2)
  (if (null? engagements)
      '()
      (change-first (lambda (acc) (eq? p1 (car acc))) engagements (cons p1 p2))))
